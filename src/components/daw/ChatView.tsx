...
import {
  getFinalVibe,
  applyVibeToEngineAndTimeline,
  generateProceduralNotesForVibe,
  livePreviewController,
  SongStructure,
  SongSection,
  defaultSongStructure,
  detectAndInsertDrops,
  adaptDropIntensity,
  createImpactDrop,
  aiDetermineOptimalDrops,
...
export function parseRequestedDuration(text: string): number | null {
  ...
  const complexMatch = lower.match(
    /(\d+)\s*(?:minutes|minute|mins|min|m)\s*(?:and\s*)?(\d+)\s*(?:seconds|second|secs|sec|s)/,
  );
  if (complexMatch) {
...
const normalizeAiAction = (action: any): AiAction => {
  ...
  if (raw.includes("TEMPO") || raw.includes("BPM") || raw.includes("KEY"))
    return "SET_GLOBAL_TEMPO_AND_KEY";
...
const normalizeAiCommand = (raw: any): AiCommand => ({
  ...
  replyText:
    typeof raw?.replyText === "string"
      ? raw.replyText
      : "I mapped that request to the closest available studio controls and I am operating the DAW now.",
...
export function ChatView() {
  ...
  const [computeTotalUsd, setComputeTotalUsd] = useState(0.2);
  ...
  const [sectionStatuses, setSectionStatuses] = useState<
    Record<string, "idle" | "generating" | "completed" | "accepted">
  >({});
  ...
  useEffect(() => {
    ...
    setSectionStatuses((prev) => {
      ...
      if (step.action === "GENERATE_DRUMS") {
        ...
        step.action === "PROCESS_VOCALS" ||
        step.action === "MIX_PROJECT" ||
        step.action === "MASTER_PROJECT" ||
        step.action === "EXPORT_PROJECT"
      ) {
  ...
  const [tactileLogs, setTactileLogs] = useState<
    { id: string; timestamp: string; action: string; status: "pending" | "success" | "failed" }[]
  >([
    {
      id: "1",
      timestamp: new Date(Date.now() - 300000).toLocaleTimeString(),
      action: "SCANNING DAW LAYOUT BOUNDARIES",
      status: "success",
    },
    {
      id: "2",
      timestamp: new Date(Date.now() - 120000).toLocaleTimeString(),
      action: "READING MIXER VOLUME dbFS CHANNELS",
      status: "success",
    },
  ]);
  ...
  const [eqAnalysis, setEqAnalysis] = useState({
    ...
    high: "0.0 dB",
    dynamics: "Normal",
    scaleCheck: "Valid harmonic key match",
  });
  ...
  >([
    {
      id: "s-1",
      name: "Ambient Harmonizer",
      timestamp: new Date(Date.now() - 600000).toLocaleTimeString(),
      status: "SUCCESS",
      solution: "Adjusted Pad synth track key to match project scale Minor C.",
    },
  ]);
  ...
  const runFullPipeline = () => {
    ...
    const queue: PipelineStep[] = [
      {
        id: "step-1",
        name: "Analyze Vocal Key & BPM",
        action: "ANALYZE_VOCAL",
        status: "idle",
        message: "Analyzing vocals, locking scale...",
      },
      {
        id: "step-2",
        name: "Synthesize Afro Drums",
        action: "GENERATE_DRUMS",
        status: "idle",
        message: "Generating syncopated Afrobeat groove...",
      },
      {
        id: "step-3",
        name: "Generate Melodic Bassline",
        action: "GENERATE_BASS",
        status: "idle",
        message: "Injecting sub-bass registers...",
      },
      {
        id: "step-4",
        name: "Structure Warm Pad Chords",
        action: "GENERATE_CHORDS",
        status: "idle",
        message: "Adding atmospheric space chord triads...",
      },
      {
        id: "step-5",
        name: "Compose Pluck Lead Harmony",
        action: "GENERATE_MELODY",
        status: "idle",
        message: "Composing syncopated Afro leads...",
      },
      {
        id: "step-6",
        name: "Trigger Swell FX risers",
        action: "GENERATE_FX",
        status: "idle",
        message: "Blending riser transitions...",
      },
      {
        id: "step-7",
        name: "Arrange Sections Timeline",
        action: "ARRANGE_SONG",
        status: "idle",
        message: "Dividing Intro/Verse/Chorus ranges...",
      },
      {
        id: "step-8",
        name: "Process Vocals FX Rack",
        action: "PROCESS_VOCALS",
        status: "idle",
        message: "Applying high sheen autotune & curves...",
      },
      {
        id: "step-9",
        name: "Mix & Balance Channels",
        action: "MIX_PROJECT",
        status: "idle",
        message: "Flipping Mixer levels & stereo split...",
      },
      {
        id: "step-10",
        name: "Loudness Master Adjust",
        action: "MASTER_PROJECT",
        status: "idle",
        message: "Normalizing mastering ceilings...",
      },
      {
        id: "step-11",
        name: "Export Master Distros",
        action: "EXPORT_PROJECT",
        status: "idle",
        message: "Compiling high-fidelity master bounces...",
      },
    ];
  ...
  const executeCommand = async (cmd: AiCommand) => {
    ...
    const addTrack = (type?: any, synthType?: any) =>
      useDawStore.getState().addTrack(type, synthType);
    const updateTrack = (id: string, updates: any) =>
      useDawStore.getState().updateTrack(id, updates);
    const addNote = (
      clipId: string,
      note: string,
      startTime: number,
      duration: number,
      isGhost?: boolean,
    ) => useDawStore.getState().addNote(clipId, note, startTime, duration, isGhost);
    ...
    const runGhostPreviewWorkflow = async (clipId: string, durationMs: number) => {
      ...
      useDawStore.setState({
        ...
        selectedTrackId: checkClipInit.trackId,
      });
      ...
      if (autoCommit) {
        ...
        if (checkClipFinal) {
          const committedNotes = (checkClipFinal.notes || []).map((n) => ({
            ...n,
            isGhost: false,
          }));
      ...
      switch (action) {
        case "SET_GLOBAL_TEMPO_AND_KEY": {
          const nextBpm = Math.min(
            180,
            Math.max(60, Number(params.bpm ?? params.tempo ?? params.targetBpm ?? 105)),
          );
          const rawKey = String(
            params.key ?? params.selectedKey ?? params.musicKey ?? projectKey ?? "C",
          )
            .replace(/\s+/g, "")
            .replace(/minor$/i, "m")
            .replace(/major$/i, "");
          const rawScale = String(
            params.scale ??
              ...
              projectScale ??
              (rawKey.toLowerCase().endsWith("m") ? "Minor" : "Major"),
          );
          const cleanKey = rawKey.replace(/m$/i, "") || "C";
          const cleanScale =
            /minor|m$/i.test(rawScale) || rawKey.toLowerCase().endsWith("m") ? "Minor" : "Major";
          const storeKey =
            cleanScale === "Minor" && !cleanKey.endsWith("m") ? `${cleanKey}m` : cleanKey;

          triggerSimulation(
            "tap",
            24,
            18,
            `Global tempo/key controls: ${nextBpm} BPM ${cleanKey} ${cleanScale}`,
          );
          setBpm(nextBpm);
          ...
          if (params.style || params.genre) {
            ...
            else if (styleName.includes("trap") || styleName.includes("drill"))
              setActiveStyle(MusicStyle.TRAP);
            else if (
              styleName.includes("pop") ||
              styleName.includes("r&b") ||
              styleName.includes("edm")
            )
              setActiveStyle(MusicStyle.POP);
        ...
        case "ANALYZE_VOCAL": {
          ...
          if (bpm < 95) {
            ...
            addLog(
              `BPM AUTO-DETECT CORRECTION: ${previousBpm} -> Amapiano (Auto-Amapiano Vibes)`,
              "success",
            );
          } else if (bpm > 120) {
            ...
            setActiveStyle(MusicStyle.TRAP);
            addLog(
              `BPM AUTO-DETECT CORRECTION: ${previousBpm} -> Trap (Auto-Trap Sub 808s)`,
              "success",
            );
          }
          ...
          if (bpm < currentProfile.bpmRange.min || bpm > currentProfile.bpmRange.max) {
            ...
            const randomBpm =
              currentProfile.bpmRange.min + Math.floor(Math.random() * (rangeDiff + 1));
            setBpm(randomBpm);
            addLog(
              `STYLE TEMPO ADJUST: Synced project BPM to style sweetspot: ${randomBpm} BPM`,
              "success",
            );
          }
          ...
          let vocalTrack = currentTracks.find(
            (t) =>
              t.type === "audio" &&
              t.clips.some((cid) => currentClips[cid] && currentClips[cid].audioUrl),
          );
          if (!vocalTrack) {
            vocalTrack = currentTracks.find(
              (t) =>
                t.name.toLowerCase().includes("vocal") ||
                t.name.toLowerCase().includes("voice") ||
                t.name.toLowerCase().includes("mic"),
            );
          }
          ...
          if (vocalClip && vocalClip.audioUrl && vocalTrack) {
            addLog(
              `🎤 ANALYZING REAL VOCALS: Extracting pitch profile & timing sync coordinates...`,
              "pending",
            );
            try {
              // Retrieve dynamic AudioBuffer
              const audioBuffer = await audioEngine.getAudioBuffer(
                vocalTrack.id,
                vocalClip.id,
                vocalClip.audioUrl,
              );
              if (audioBuffer) {
                ...
                if (Math.abs(estimatedBpm - bpmVal) > 1.5) {
                  addLog(
                    `⌛ VOCAL SYNC WARPING: Detected native vocal BPM of ${estimatedBpm} BPM. Elastic-warping vocals to project BPM (${bpmVal} BPM) with pitch-lock...`,
                    "pending",
                  );
                  const warpSuccess = await audioEngine.warpClipAudio(
                    vocalTrack.id,
                    vocalClip.id,
                    bpmVal,
                    estimatedBpm,
                  ...
                  if (warpSuccess) {
                    ...
                    addLog(
                      `✅ VOCALS ELASTICALLY WARPED: Phase-coherent WSOLA time-stretch completed. Pitch locked and synchronized!`,
                      "success",
                    );
                  }
                } else {
                  addLog(
                    `✅ VOCAL BPM SYNCED: Vocals match project tempo (${bpmVal} BPM). No time-stretching required!`,
                    "success",
                  );
                }
                ...
                if (analyzedNotes && analyzedNotes.length > 0) {
                  ...
                  try {
                    const { VocalAnalyzerProcessor } =
                      await import("../../audio/vocal-analyzer-processor");
                    const duration16ths = vocalClip.duration || 32;
                    const detectedPhrases = VocalAnalyzerProcessor.detectPhrases(
                      analyzedNotes,
                      duration16ths,
                    );
                    ...
                    if (detectedPhrases.length > 0) {
                      addLog(
                        `🔍 VOCAL MIR ENGINE: Extracting vocal phrase structures...`,
                        "pending",
                      );
                      const mappedStructure = VocalAnalyzerProcessor.mapPhrasesToSongStructure(
                        detectedPhrases,
                        duration16ths,
                      );
                      useDawStore.getState().setSongStructure(mappedStructure);
                      addLog(
                        `🌟 VOCAL PHRASE ARRANGEMENT LOCKED: Sequencer adapted timeline sections to fit your recorded phrases perfectly!`,
                        "success",
                      );
                    }
                  } catch (mirErr) {
                  ...
                  const activeValues = analyzedNotes
                    .filter((n) => !n.isSilence)
                    .map((n) => n.midi % 12);
                  if (activeValues.length > 0) {
                    const counts = Array(12).fill(0);
                    activeValues.forEach((m) => counts[m]++);

                    const majorScaleStrength = [
                      6.3, 2.2, 3.4, 2.3, 4.3, 4.0, 2.5, 5.1, 2.3, 3.6, 2.2, 2.8,
                    ];
                    const minorScaleStrength = [
                      6.3, 2.6, 3.5, 5.3, 2.6, 3.5, 2.5, 4.7, 3.9, 2.6, 3.3, 3.1,
                    ];
                    ...
                    const storeKey =
                      detectedScaleType === "Minor" ? `${detectedKey}m` : detectedKey;
                    setProjectKey(storeKey);
                    setProjectScale(detectedScaleType);

                    addLog(
                      `🎤 VOCALS SIGNAL KEY DETECTED: "${detectedKey} ${detectedScaleType}"`,
                      "success",
                    );
                    addLog(
                      `⚡ Locked global instrument generators and scale correction nodes to: ${detectedKey} ${detectedScaleType}. Perfect harmony locked!`,
                      "success",
                    );
                  }
                }
              ...
              console.warn(
                "Deeper pitch profile extraction had an issue (handled gracefully):",
                err,
              );
            }
          }
          ...
          if (!wasVocalPitchAnalyzed) {
            ...
            addLog(
              `ℹ️ No recorded vocals found on timeline. Initializing system defaults: ${detectedKey} ${detectedScaleType}`,
              "success",
            );
          }
          ...
          if (!vocalTrack) {
            ...
            if (newest) {
              ...
              const clipId = useDawStore
                .getState()
                .addClip(
                  newest.id,
                  0,
                  undefined,
                  [0.1, 0.4, 0.8, 0.5, 0.9, 0.6, 0.3, 0.8, 0.9, 0.4, 0.1, 0.05],
                  32,
                );
            }
          } else {
            ...
            vocalTrack.clips.forEach((cid) => {
              ...
              if (clip) {
                ...
                if (originalStart !== snappedStart) {
                  addLog(
                    `⚡ VOCAL SYNCHRONIZER: Grid-aligned vocals starting step from ${originalStart.toFixed(1)} to ${snappedStart} for zero-latency rhythmic beat alignment!`,
                    "success",
                  );
                }
              }
          ...
          setEqAnalysis((prev) => ({
            ...prev,
            scaleCheck: `Vocals perfectly aligned in ${detectedKey} ${detectedScaleType}`,
          }));
          break;
        }
        ...
        case "GENERATE_FX": {
          ...
          if (newest) {
            ...
            const clipId = useDawStore
              .getState()
              .addClip(newest.id, 24, undefined, [0.01, 0.05, 0.1, 0.3, 0.45, 0.6, 0.85, 1.0], 8);
        ...
        case "ARRANGE_SONG": {
          ...
          if (lastUserMsg) {
            ...
            if (parsed !== null) {
              ...
              addLog(
                `CUSTOM TIME TARGET ACQUIRED: Structuring layout for a precise ${Math.floor(targetSeconds / 60)}m ${targetSeconds % 60}s song progression...`,
                "pending",
              );
            } else {
              addLog(
                `AUTO-ARRANGE INITIATED: Applying standard premium radio length of 2:00 (120 seconds)...`,
                "pending",
              );
            }
          } else {
          ...
          const introEnd = Math.max(1, Math.round(totalClips * 0.1)); // Intro (~10%)
          ...
          addLog(
            `Designing arrangement bounds with ${totalClips} loop blocks [Intro (0-${introEnd * 32}) | Verse 1 (${introEnd * 32}-${verse1End * 32}) | Pre-Chorus (${verse1End * 32}-${preChorusEnd * 32}) | Chorus 1 (${preChorusEnd * 32}-${chorus1End * 32}) | Verse 2 (${chorus1End * 32}-${verse2End * 32}) | Bridge (${verse2End * 32}-${bridgeEnd * 32}) | Chorus 2 (${bridgeEnd * 32}-${chorus2End * 32}) | Outro]...`,
            "pending",
          );
          ...
          for (const track of activeTracks) {
            ...
            const isEmptyMidi =
              isMidiTrack && (!masterClip || !masterClip.notes || masterClip.notes.length === 0);
            ...
            if (!masterClip || isEmptyMidi) {
              ...
              if (
                ...
                track.name.toLowerCase().includes("sub")
              ) {
                synthPreset = "synthbass";
              } else if (
                track.name.toLowerCase().includes("chord") ||
                track.name.toLowerCase().includes("pad") ||
                track.name.toLowerCase().includes("keys")
              ) {
                synthPreset = "poly";
              } else {
                synthPreset = "pluck";
            ...
            for (let c = 0; c < totalClips; c++) {
              ...
              if (shouldAdd_c && masterClip) {
                ...
                const idClip = latestStore.addClip(
                  track.id,
                  startTick,
                  masterClip.audioUrl,
                  masterClip.recordingPeaks,
                  masterClip.duration,
                );

                // Copy vocalNotes if they exist (vital for audio tracks!)
                if (masterClip.vocalNotes && masterClip.vocalNotes.length > 0) {
                  latestStore.updateClip(idClip, {
                    vocalNotes: JSON.parse(JSON.stringify(masterClip.vocalNotes)),
                  });
                }
          ...
          addLog(
            "AI is running post-production timeline check to align, snap, and heal any clip gaps...",
            "pending",
          );
          useDawStore.setState({
            aiStepMessage: "Healing arrangement gaps and snapping clips... ✨",
          });
          await new Promise((r) => setTimeout(r, 600));
          ...
          for (const track of allTracksToHeal) {
            ...
            const trackClips = Object.values(allClipsBeforeHeal).filter(
            ...
            let primaryClip =
              trackClips.find((clip) => clip.notes && clip.notes.length > 0) || trackClips[0];
            ...
            for (let c = 0; c < totalClips; c++) {
              ...
              const currentClips = Object.values(useDawStore.getState().clips).filter(
                (clip) => clip.trackId === track.id,
              );
              ...
              if (!hasClipInBlock) {
                console.log(
                  `AI healing gap: Inserting connector clip on track ${track.name} at tick ${startTick}`,
                );
                const idClip = finalState.addClip(
                  track.id,
                  startTick,
                  primaryClip?.audioUrl,
                  primaryClip?.recordingPeaks,
                  32,
                );
          ...
          addLog(
            "Arrangement timeline verified & healed! All clips are perfectly snapped, continuous, and gapless.",
            "success",
          );
          useDawStore.setState({ aiActivePulseTrackId: null });
          break;
        ...
        case "PROCESS_VOCALS": {
          ...
          const targetVocal = useDawStore
            .getState()
            .tracks.find(
              (t) =>
                t.name.toLowerCase().includes("vocal") || t.name.toLowerCase().includes("voice"),
            );
          if (targetVocal) {
            ...
            const fxPreset = {
              ...
              delay: { enabled: true, time: "8n", feedback: 0.3, mix: 0.15 }, // stereo ping-pong delays
            ...
            setEqAnalysis((prev) => ({
              ...prev,
              dynamics: `DSP Auto-tune engine locked to ${scaleDisplay} with high-sheen saturation.`,
            }));
            addLog(
              `🎚️ VOCAL AUTO-TUNE APPLIED: Vocal performance pitch corrected to ${scaleDisplay}`,
              "success",
            );
          } else {
        ...
        case "MIX_PROJECT": {
          ...
          for (const track of allTracks) {
            ...
            if (track.name.toLowerCase().includes("vocal")) {
              ...
              track.name.toLowerCase().includes("drum") ||
              track.name.toLowerCase().includes("groove")
            ) {
              updatedVol =
                activeStyle === MusicStyle.AMAPIANO
                  ? -5.5
                  : activeStyle === MusicStyle.POP
                    ? -6.0
                    : -7.0;
              updatedPan = 0;
            } else if (
              ...
              track.name.toLowerCase().includes("pad") ||
              track.name.toLowerCase().includes("atmos") ||
              track.name.toLowerCase().includes("rhodes")
            ) {
              updatedVol = -13.0;
              updatedPan = -0.28; // stereo separation left
            } else if (
              track.name.toLowerCase().includes("melody") ||
              track.name.toLowerCase().includes("pluck")
            ) {
              updatedVol = -11.5;
              updatedPan = 0.26; // stereo separation right
            } else if (
              track.name.toLowerCase().includes("fx") ||
              track.name.toLowerCase().includes("sweeps")
            ) {
              updatedVol = -16.5;
              updatedPan = 0.12;
            }
            ...
            if (activeStyle === MusicStyle.AMAPIANO) {
              ...
              if (
                track.name.toLowerCase().includes("bass") ||
                track.name.toLowerCase().includes("sub") ||
                track.name.toLowerCase().includes("808")
              ) {
          ...
          setEqAnalysis((prev) => ({
            ...prev,
            dynamics: "Stereo fields optimized. Sweeper pans locked.",
          }));
          break;
        }
        ...
        case "SEGMENTED_GENERATE": {
          ...
          addLog(
            `⚡ SEGMENTED PRODUCTION STREAM: Launching segment chain generation pipeline...`,
            "pending",
          );
          try {
            const { generateAndAssembleSegmentedTrack } =
              await import("../../utils/segmentedGenerator");
            ...
            addLog(
              `✅ FULL 2-MINUTE STRUCTURE COMPILED: Seamlessly stitched ${targetDurationSeconds}s of consistent vibe audio segments on timeline!`,
              "success",
            );
          } catch (err) {
        ...
        case "INSERT_CUSTOM_NOTES": {
          ...
          const currentTracks = useDawStore.getState().tracks;
          let targetTrack = currentTracks.find((t) =>
          ...
          if (targetTrack) {
            ...
            if (notes && Array.isArray(notes) && notes.length > 0) {
              ...
              addLog(
                `GENERATED ${notes.length} CUSTOM NOTES ON TRACK [${targetTrack.name}]`,
                "success",
              );
            } else {
        ...
        case "UPDATE_MIXER": {
          ...
          for (const track of allTracks) {
            if (
              trackSearch.toLowerCase() === "all" ||
              track.name.toLowerCase().includes(trackSearch.toLowerCase())
            ) {
              useDawStore.setState({
                aiActivePulseTrackId: track.id,
                aiStepMessage: `Aligning gain fader: ${track.name}`,
              });
        ...
        case "APPLY_FX_RACK": {
          ...
          if (targetTrack) {
            ...
            addLog(
              `TUNED DSP EFFECT NODE [${fxType.toUpperCase()}] ON ${targetTrack.name}`,
              "success",
            );
          } else {
        ...
        case "WRITE_STEP_SEQUENCER": {
          ...
          const currentTracks = useDawStore.getState().tracks;
          let targetTrack =
            currentTracks.find((t) => t.synthType === "membrane") ||
            currentTracks.find((t) => t.type === "midi");
          if (!targetTrack) {
          ...
          if (targetTrack) {
            ...
            if (notes && Array.isArray(notes) && notes.length > 0) {
              ...
              addLog(
                `SEQUENCED ${notes.length} DRUM TRANSIENTS ON STEP-SEQUENCER [${targetTrack.name}]`,
                "success",
              );
            } else {
        ...
        case "APPLY_AUTOMATION_CLIP": {
          ...
          if (targetTrack) {
            ...
            updateTrack(targetTrack.id, {
              automationType: type,
              automationCurve:
                curve && Array.isArray(curve) && curve.length === 16 ? curve : Array(16).fill(0.8),
              automationEnabled: enabled,
              fx: trackFx,
            });

            addLog(
              `MAPPED AND ARMED AUTOMATION ENVELOPE: [${type.toUpperCase()}] CYCLE`,
              "success",
            );
          }
          await new Promise((r) => setTimeout(r, 350));
        ...
        case "SET_PORTAMENTO_GLIDE": {
          ...
          const currentTracks = useDawStore.getState().tracks;
          const targetTrack =
            currentTracks.find((t) => t.name.toLowerCase().includes(trackSearch.toLowerCase())) ||
            currentTracks[0];
          if (targetTrack) {
            useDawStore.setState({ aiActivePulseTrackId: targetTrack.id });
            updateTrack(targetTrack.id, { portamento });
            addLog(
              `SET SYNTH PORTAMENTO GLIDE ON [${targetTrack.name}] TO ${portamento}s`,
              "success",
            );
          }
          await new Promise((r) => setTimeout(r, 350));
        ...
        case "TOGGLE_SIDECHAIN": {
          ...
          if (targetTrack) {
            useDawStore.setState({ aiActivePulseTrackId: targetTrack.id });
            const currentSidechain = targetTrack.fx?.sidechain || {
              enabled: false,
              ratio: 4,
              threshold: -20,
              release: 0.12,
            ...
            addLog(
              `SIDECHAIN KICK PUMPING ${enabled ? "ENABLED" : "DISABLED"} FOR [${targetTrack.name}]`,
              "success",
            );
          }
          await new Promise((r) => setTimeout(r, 350));
        ...
        case "SET_TIME_SHAPER": {
          ...
          if (targetTrack) {
            ...
            addLog(
              `GROSS BEAT / TIME-SHAPER [${mode.toUpperCase()}] ACTIVE ON [${targetTrack.name}]`,
              "success",
            );
          }
          await new Promise((r) => setTimeout(r, 350));
        ...
        case "APPLY_PEAK_CONTROLLER": {
          ...
          const targetTrack =
            currentTracks.find((t) => t.name.toLowerCase().includes(trackSearch.toLowerCase())) ||
            currentTracks[0];
          const sourceTrack = currentTracks.find((t) =>
            t.name.toLowerCase().includes(sourceTrackSearch.toLowerCase()),
          );
          ...
          if (targetTrack && sourceTrack) {
            ...
            addLog(
              `PEAK CONTROLLER CONFIG: MODULATION ON [${targetTrack.name}] DRIVEN BY [${sourceTrack.name}] FOR [${targetParam.toUpperCase()}]`,
              "success",
            );
          } else if (targetTrack) {
            addLog(
              `PEAK CONTROLLER ERROR: SOURCE CARRIER TRACK [${sourceTrackSearch}] NOT FOUND`,
              "failed",
            );
          }
          await new Promise((r) => setTimeout(r, 350));
        ...
        default:
          ...
          throw new Error(
            `The action command identifier "${action}" is unrecognized by physical studio interface.`,
          );
      }
  ...
  const proceedWithAIGeneration = async (userMessage: string) => {
    ...
    try {
      ...
      // Look up the admin-configured active AI provider so the gate + charge
      // use that provider's per-prompt cost (not a hardcoded $0.20).
      // Falls back to $0.20 if no active provider is configured.
      let providerCost = 0.2;
      ...
      try {
        ...
        if (activeProviders && activeProviders.length > 0) {
          ...
          providerCost = activeProvider.cost_per_prompt_usd || 0.2;
        }
      } catch (lookupErr) {
        console.warn("[ChatView] provider lookup failed, using default $0.20 cost:", lookupErr);
      }

      // HARD insufficient-funds gate — no auto-grants, no demo vouchers.
      // Gate threshold scales with the active provider's per-prompt cost.
      if (currentUsd < providerCost) {
        addChatMessage({
          role: "assistant",
          content: `🚫 **Insufficient funds**: Your wallet balance is $${currentUsd.toFixed(2)} (₦${currentNaira.toLocaleString()}). This AI prompt costs **$${providerCost.toFixed(2)}**. Please fund your wallet to continue producing.\n\n👉 Open **Wallet** from the menu to top up.`,
        });
        setIsLoading(false);
        return;
      }
      ...
      const updatedUsd = currentUsd - providerCost;
      const updatedNaira = currentNaira - Math.round(providerCost * 1600);
        ...
        responseSchema: {
          ...
          properties: {
            action: {
              type: "STRING",
              description:
                "The AI action command identifier. Must be one of the specified actions.",
            },
            params: {
              type: "OBJECT",
            ...
            rationale: {
              type: "STRING",
              description:
                "Brief real-time description of the human substitute physical action on the board.",
            },
            replyText: {
              type: "STRING",
              description:
                "The rich textual chat response, explanation, answer, or question to show the user.",
            },
            customPipeline: {
              type: "ARRAY",
              description:
                "An optional sequence of dynamic production steps for multi-step automated DAW operations. Plan precisely where to start, what tracks to add, which notes to write, and how to mix/master, bypassing all hardcoded constraints.",
              items: {
                type: "OBJECT",
                properties: {
                  id: { type: "STRING" },
                  name: {
                    type: "STRING",
                    description:
                      "The name/title of this custom step (e.g., 'Lay Down Afro Percussion Groove')",
                  },
                  action: {
                    type: "STRING",
                    description:
                      "The single DAW action to run for this step (one of the valid action names)",
                  },
                  params: {
                    type: "OBJECT",
                    description: "Exact parameters map to configure for this custom step",
                  },
                  message: {
                    type: "STRING",
                    description:
                      "Dynamic progress message to overlay in the HUD during this step (e.g. 'Programming custom polyphonic minor chords...')",
                  },
                },
      ...
      // Call our server route — wallet charge already happened above.
      // The second provider lookup + charge that used to live here was removed
      // because it caused a DOUBLE-CHARGE per prompt. Provider info was already
      // resolved earlier (activeProvider + providerCost) for the gate + charge.
      let response;
      let usedSupabaseEdge = false;
      let feeCharged = `$${providerCost.toFixed(2)} / ₦${Math.round(providerCost * 1600)}`;
      ...
      try {
        ...
        if (user && !activeProvider) {
          // Fallback path: provider lookup above failed (network blip, etc.).
          // Try once more here purely to populate the fee label.
          ...
          if (!reqErr && activeProviders && activeProviders.length > 0) {
            activeProvider = activeProviders.find((p) => p.is_default) || activeProviders[0];
            const pc = activeProvider.cost_per_prompt_usd || 0.2;
            feeCharged = `$${pc.toFixed(2)} / ₦${Math.round(pc * 1600)}`;
          }
        }
      } catch (edgeErr: any) {
        console.warn("[ChatView] secondary provider lookup failed:", edgeErr);
      }
      ...
      if (!usedSupabaseEdge) {
        ...
        const res = await apiFetch("/api/ai/chat", {
          ...
          body: JSON.stringify({
            ...
            systemInstruction,
            config,
            // Send only the provider id. The server looks up the rest of the
            // provider config (endpoint / model / api_key) using its
            // SUPABASE_SERVICE_ROLE_KEY, so admin API keys never reach the
            // client and never travel through request bodies.
      ...
      if (
        cmd &&
        cmd.action !== "CHAT_AND_EXPLAIN" &&
        cmd.action !== "FULL_AUTO_PRODUCE" &&
        (!cmd.customPipeline || cmd.customPipeline.length === 0)
      ) {
        ...
        setSolutionHistory((prev) => [
          {
            ...
            solution:
              cmd.rationale ||
              `Success: Automated physical substitute ${humanActionName} was completed on the DAW console.`,
          },
          ...prev,
        ]);
      } catch (execErr: any) {
  ...
  const handleSend = async (overrideInput?: any) => {
    ...
    if (isContinuationRequest) {
      ...
      if (pipelineQueue.length === 0) {
        addChatMessage({
          role: "assistant",
          content:
            'ℹ️ **AI PRODUCER**: No active song production queue was detected in this session. If you want me to build a clean song design, just ask me to **"produce an Afrobeat song"** or **"automatically produce a full song"**!',
        });
        return;
      }
      ...
      if (firstUncompletedIndex === -1) {
        addChatMessage({
          role: "assistant",
          content:
            "🎉 **AI PRODUCER**: All tracks and modules in the song production pipeline are already fully completed, mixed, and mastered! If you would like to start a new sequence design, feel free to let me know.",
        });
        return;
      }
    ...
    if (
      lowerMsg.includes("amapiano") ||
      lowerMsg.includes("log drum") ||
      lowerMsg.includes("bouncing bacardi") ||
      lowerMsg.includes("piano keys")
    ) {
      detectedStyle = MusicStyle.AMAPIANO;
    } else if (
      lowerMsg.includes("trap") ||
      lowerMsg.includes("808") ||
      lowerMsg.includes("bell pluck") ||
      lowerMsg.includes("subbass")
    ) {
      detectedStyle = MusicStyle.TRAP;
    } else if (
      lowerMsg.includes("afro") ||
      lowerMsg.includes("afrobeat") ||
      lowerMsg.includes("groove bass")
    ) {
      detectedStyle = MusicStyle.AFROBEATS;
    } else if (
      lowerMsg.includes("pop") ||
      lowerMsg.includes("synthlead") ||
      lowerMsg.includes("dance") ||
      lowerMsg.includes("sing") ||
      lowerMsg.includes("r&b") ||
      lowerMsg.includes("edm")
    ) {
  ...
  return (
    <div
      className={`h-full flex-1 flex flex-col overflow-hidden select-none border-l border-[#1A1A1A] relative transition-all duration-500 ${pipelineActive ? "bg-transparent shadow-none border-transparent pointer-events-none" : isWorking ? "bg-transparent shadow-none border-transparent" : "bg-[#050505]"}`}
    ...
    >
          ...
          >
            ...
            <div
              className={`w-10 h-10 rounded-full flex items-center justify-center ${
                pointer.type === "tap"
                  ? "bg-[#00FF9C]/20 border border-[#00FF9C]"
                  : pointer.type === "swipe"
                    ? "bg-pink-500/20 border border-pink-500 animate-pulse"
                    : pointer.type === "scroll"
      ...
      <div
        className={`flex flex-col flex-1 min-h-0 transition-all duration-500 ${pipelineActive ? "opacity-0 pointer-events-none" : "opacity-100"}`}
      >
          ...
          <div className="bg-amber-500/10 border-b border-amber-500/30 text-amber-200 py-3 px-4 font-mono font-bold text-xs uppercase flex items-center justify-between shadow-[0_4px_25px_rgba(245,158,11,0.15)] z-40 shrink-0 gap-3 relative animate-in slide-in-from-top duration-300">
            ...
            <button
              onClick={async () => {
                ...
                if (success) {
                  setIsComputePaused(false);
                  setComputeTotalUsd((prev) => prev + 0.2);
        ...
        <div className="h-9 bg-[#0b0c0d] border-b border-[#1A1A1A] px-4 flex items-center justify-between text-[11px] font-mono">
          <div className="flex items-center space-x-2 text-gray-400">
            ...
            <span className="text-pink-400 font-bold uppercase text-[9px]">
              FEE: $0.20 (₦320) PER OPERATIONAL COMMAND
            </span>
          </div>
          <div className="flex items-center space-x-2.5 text-white">
            ...
            {walletLoading ? (
              ...
              <span className="text-[#00FF9C] font-semibold">
                Bal: ${wallet ? Number(wallet.balance_usd).toFixed(2) : "0.00"} (₦
                {wallet ? Number(wallet.balance_naira).toLocaleString() : "0"})
              </span>
            )}
            <button
              onClick={() => {
          ...
          >
            <div className="flex items-center space-x-2 text-[#00FF9C]">
              ...
              <span className="font-mono text-[10px] uppercase font-bold tracking-widest">
                AI Guardrails & Permission States
              </span>
            </div>
              ...
              >
                <div className="grid grid-cols-2 lg:grid-cols-4 gap-2 pt-1">
                  {[
                    ...
                    {
                      key: "preventDeletionWithoutConfirmation",
                      label: "Block deletion without confirm",
                      colSpan: 2,
                      isBlockRule: true,
                    },
                    {
                      key: "preventOverwritingWithoutBackup",
                      label: "Block overwrite without backup",
                      colSpan: 2,
                      isBlockRule: true,
                    },
                    {
                      key: "aiWalletSpend",
                      label: "AI Wallet Auto-Spend",
                      colSpan: 2,
                      color: "text-amber-400 font-extrabold shadow-[0_0_10px_rgba(245,158,11,0.1)]",
                      isWalletPerm: true,
                    },
                  ].map((perm) => (
                    <div
                    ...
                    >
                      <div className="flex flex-col">
                        <span
                          className={`text-[10px] font-mono leading-none ${perm.color || "text-slate-300"} font-bold uppercase`}
                        >
                          {perm.label}
                        </span>
                        <span className="text-[8px] mt-1 uppercase font-mono">
                          {perm.isBlockRule ? (
                            ...
                            <span className="text-amber-500/95 font-bold animate-pulse">
                              Auto-Debit Approved
                            </span>
                          ) : (
          ...
          <div className="flex items-center space-x-3">
            ...
            <div>
              <h2 className="text-sm font-bold tracking-wider uppercase text-white">
                AI Production Studio Systems
              </h2>
              <p className="text-[10px] text-gray-500 font-mono">
                Active Physical Substitute Engine • Model 3.5-Flash
              </p>
            </div>
          </div>
          ...
          <AnimatePresence mode="wait">
            ...
            {subTab === "assistant" && (
              ...
              >
                  ...
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-1.5">
                      ...
                      <span className="text-[11px] font-bold text-white uppercase tracking-wider font-mono">
                        Ghost Preview Engine
                      </span>
                    </div>
                    <span
                      className={`text-[9px] font-mono px-2 py-0.5 rounded font-extrabold tracking-wider ${
                        aiPreviewAutoCommit
                ...
                <div
                  id="composition-plan-panel"
                  className="bg-[#0e1012] border border-white/5 rounded-2xl p-4 flex flex-col gap-3 shrink-0"
                >
                  <div
                  ...
                  >
                    ...
                    <div className="flex items-center gap-2">
                      <span className="text-[9px] font-mono text-gray-500 bg-white/5 px-2 py-0.5 rounded-md">
                        ...
                        {songStructure?.sections?.length || 6} Locked
                      </span>
                      {isPlanOpen ? (
                  ...
                  {isPlanOpen && (
                    ...
                    >
                      <div className="text-[10px] text-gray-400 mb-1 leading-relaxed font-sans normal-case whitespace-normal">
                        The AI carefully structures and produces notes for each section of your
                        track. Approve the parts you love or refine them dynamically.
                      </div>
                          ...
                          return (
                            ...
                            >
                              <div className="flex items-center gap-2.5 min-w-0 pr-2">
                                {/* Glowing state bead */}
                                <div
                                  className={`w-1.5 h-1.5 rounded-full ${glowClass} ${
                                    status === "generating"
                                ...
                                <div className="flex flex-col min-w-0">
                                  ...
                                  <span className="text-[8px] font-mono text-zinc-500">
                                    Bar {sec.startBar} - {sec.startBar + sec.lengthBars} (
                                    {sec.lengthBars} Bars)
                                  </span>
                                </div>
                              </div>
                              ...
                              <div className="flex items-center gap-1.5 shrink-0">
                                <span
                                  className={`text-[8px] font-mono font-black uppercase tracking-wider py-0.5 px-1.5 rounded-md border ${statusColor}`}
                                >
                                  {statusLabel}
                                </span>
                ...
                <div className="bg-[#0e1012] border border-white/5 rounded-2xl p-3.5 flex flex-col gap-2 shrink-0">
                  ...
                  <p className="text-[10px] text-gray-500 font-sans leading-relaxed">
                    Adapt the template, arrangement, mixing algorithms, instrument scales, log
                    drum/808 ratios, and generation structures dynamically to target a genre or
                    vibe.
                  </p>
                ...
                {pipelineActive && pipelineQueue.length > 0 && (
                  <div className="bg-[#0b0c0d] border border-emerald-500/20 rounded-2xl p-4 shadow-xl shadow-emerald-950/5 relative overflow-hidden shrink-0">
                    <div className="flex items-center justify-between mb-3 text-[10px] font-mono text-emerald-400">
                      ...
                      <span>
                        Step {pipelineQueue.findIndex((s) => s.status === "running") + 1} /{" "}
                          ...
                          className={`flex items-center justify-between rounded-xl px-3 py-2 border transition-all ${
                            step.status === "running"
                              ...
                              : step.status === "completed"
                                ...
                                : "bg-black/40 border-white/5 opacity-35"
                          }`}
                        >
                ...
                {!pipelineActive &&
                  ...
                  pipelineQueue.some((s) => s.status === "completed") &&
                  pipelineQueue.some((s) => s.status !== "completed") && (
                    <div className="bg-[#120f0e] border border-amber-500/20 rounded-2xl p-4 shadow-xl shadow-amber-950/5 relative overflow-hidden shrink-0 animate-in fade-in duration-300">
                      <div className="flex items-center justify-between mb-3 text-[10px] font-mono text-amber-500">
                        ...
                        <span>
                          Stopped at Step{" "}
                          {pipelineQueue.findIndex((s) => s.status !== "completed") + 1} /{" "}
                          {pipelineQueue.length}
                        </span>
                      </div>

                      <p className="text-[10px] text-zinc-400 font-sans mb-3 leading-relaxed">
                        The AI song production was interrupted or stopped. You can ask the AI to
                        **"continue"** in the chat, or click the button below to resume from where
                        it stopped!
                      </p>
                        ...
                        <button
                          onClick={() => {
                            ...
                            addChatMessage({
                              role: "assistant",
                              content:
                                "🗑️ Song production queue cleared. You can now start afresh or prompt new creations!",
                            });
                          }}
                  ...
                  <div className="flex-1 flex flex-col items-center justify-center p-8 text-center max-w-sm mx-auto space-y-4">
                    ...
                    <div>
                      <h3 className="text-sm font-bold text-white uppercase tracking-wider">
                        Awaiting Executive Commands
                      </h3>
                      <p className="text-xs text-gray-500 mt-1 leading-relaxed">
                        Your AI DAW Agent is primed to physical actions. Prompt "produce full
                        Afrobeat song automatically", "generate syncopated drums", "master
                        compilation", or toggle guardrail levels above!
                      </p>
                    </div>
                  </div>
                )}
                ...
                {chatMessages.map((msg, i) => (
                  ...
                  >
                    <div
                      className={`flex max-w-[90%] ${msg.role === "user" ? "flex-row-reverse" : "flex-row"} items-start gap-3`}
                    >
                      <div
                        className={`mt-0.5 w-8 h-8 shrink-0 rounded-xl flex items-center justify-center border font-mono ${
                          msg.role === "user"
                      ...
                      <div className="relative group flex flex-col items-start message-bubble">
                        <div
                          className={`rounded-2xl px-4 pt-3 pb-8 pr-4 text-xs leading-relaxed whitespace-pre-wrap relative ${
                            msg.role === "user"
                ...
                {isLoading && (
                  <div
                    className={`bg-[#0b0c0f] border ${adRequiredPrompt ? "border-amber-500/30" : "border-emerald-500/20"} rounded-2xl p-5 shrink-0 space-y-3 font-mono`}
                  >
                    <div className="flex items-center justify-between text-[10px] text-zinc-400 font-extrabold uppercase tracking-widest">
                      <span
                        className={`flex items-center gap-2 ${adRequiredPrompt ? "text-amber-400" : "text-emerald-400"}`}
                      >
                        <span className="relative flex h-2 w-2">
                          <span
                            className={`animate-ping absolute inline-flex h-full w-full rounded-full ${adRequiredPrompt ? "bg-amber-400" : "bg-emerald-400"} opacity-75`}
                          ></span>
                          <span
                            className={`relative inline-flex rounded-full h-2 w-2 ${adRequiredPrompt ? "bg-amber-500" : "bg-emerald-500"}`}
                          ></span>
                        </span>
                        {isComputePaused
                          ? "⏸️ COMPUTE PAUSED"
                          : adRequiredPrompt
                            ? "📺 STANDBY: WAITING FOR REWARDED AD"
                            : "⚙️ DYNAMIC COMPUTE RUNNING"}
                      </span>
                      <span>
                        {adRequiredPrompt ? (
                          ...
                          <>
                            Spent:{" "}
                            <span className="text-[#00FF9C] font-semibold">
                              ${computeSpentUsd.toFixed(2)}
                            </span>{" "}
                            / ${computeTotalUsd.toFixed(2)}
                          </>
                        )}
                      </span>
                    </div>
                    ...
                    <div className="w-full h-2 bg-zinc-950 rounded-full overflow-hidden border border-white/5 relative">
                      <div
                        ...
                        style={{
                          width: adRequiredPrompt
                            ? "100%"
                            : `${Math.min(100, (computeSpentUsd / computeTotalUsd) * 100)}%`,
                        }}
                      />
                    </div>
                ...
                {adRequiredPrompt && (
                  <div className="bg-[#121316] border border-amber-500/20 rounded-2xl p-5 shrink-0 flex flex-col gap-4 font-mono shadow-[0_0_35px_rgba(245,158,11,0.05)]">
                    ...
                    <p className="text-[10px] text-zinc-400 font-sans leading-relaxed">
                      Since you are on the standard plan, you must view a Google AdMob Rewarded
                      Video Ad before subtracting your{" "}
                      <span className="text-white font-bold">$0.20 workspace fee</span> and starting
                      AI synthesis.
                      <br />
                      <br />
              ...
              >
                <div className="bg-[#121314] rounded-2xl border border-[#222325] p-4 relative overflow-hidden">
                  <div className="flex items-center justify-between mb-3 text-[10px] font-mono">
                    <div className="flex items-center space-x-2 text-gray-400">
                      <span
                        className={`w-1.5 h-1.5 rounded-full ${visionScanOn ? "bg-red-500 animate-pulse" : "bg-gray-600"}`}
                      />
                    ...
                    <div className="flex-1 flex gap-1 items-stretch py-1">
                      <div className="w-16 border-r border-white/5 flex flex-col gap-1 justify-around py-1">
                        {tracks.map((t) => (
                          <div
                            key={t.id}
                            className="h-3 rounded-[3px] bg-white/5 px-1 flex items-center justify-between text-[7px] text-gray-400"
                          >
                      ...
                      <div className="flex-1 flex flex-col gap-1 justify-around py-1 relative">
                        {tracks.map((t) => (
                          <div
                            key={t.id}
                            className="h-3 bg-[#111] rounded-[3px] border border-white/5 relative flex items-center"
                          >
                ...
                <div>
                  <h3 className="text-[11px] font-mono text-gray-400 uppercase tracking-widest mb-2 font-bold flex items-center justify-between">
                    ...
                    <span className="text-pink-400 text-[9px] uppercase font-semibold">
                ...
                <div className="space-y-2">
                  ...
                  <div className="space-y-2 max-h-48 overflow-y-auto scrollbar-hide">
                    {solutionHistory.map((sol) => (
                      <div
                        key={sol.id}
                        className="p-3 bg-[#0d0E10] border border-white/5 rounded-xl flex items-start gap-2.5"
                      >
                ...
                <div className="bg-[#0B0C0E] rounded-xl p-3 border border-white/5">
                  ...
                  <div className="font-mono text-[9px] text-[#00FF9C] space-y-1 max-h-24 overflow-y-auto scrollbar-hide">
                    {tactileLogs.map((log) => (
                      <div
                        key={log.id}
                        className="flex justify-between hover:bg-white/5 p-0.5 rounded"
                      >
              ...
              >
                <div className="bg-[#121314] rounded-2xl border border-[#222325] p-4 font-mono">
                  <div className="flex items-center justify-between mb-2 text-[10px]">
                    ...
                    <span className="text-[#00FF9C] uppercase">
                      {playbackState === "playing" ? "ANALYSIS STREAMING ON" : "IDLE WAIT"}
                    </span>
                  </div>
                ...
                <div className="grid grid-cols-2 gap-2">
                  <div className="bg-[#101010] p-3 rounded-xl border border-white/5">
                    <span className="text-[8px] font-mono text-slate-500 uppercase font-bold">
                      Project Tempo Tracker
                    </span>
                    <div className="text-lg font-bold text-white mt-1 uppercase font-mono">
                      {bpm} BPM
                    </div>
                  </div>
                  <div className="bg-[#101010] p-3 rounded-xl border border-white/5">
                    <span className="text-[8px] font-mono text-slate-500 uppercase font-bold text-emerald-400">
                      Root Key Signature
                    </span>
                    <div className="text-lg font-bold text-[#00FF9C] mt-1 font-mono">
                  ...
                  <div className="bg-[#101010] p-3 rounded-xl border border-white/5 col-span-2 flex justify-between items-center">
                    <div>
                      <span className="text-[8px] font-mono text-slate-500 uppercase font-bold">
                        Calculated harmonic health
                      </span>
                      <div className="text-[10px] font-medium text-white mt-0.5">
                        {eqAnalysis.scaleCheck}
                      </div>
                    </div>
                ...
                <div className="bg-[#0D0E0F] rounded-xl p-4 border border-white/5 space-y-3">
                  <h3 className="text-[10px] font-mono text-gray-400 uppercase tracking-widest font-bold">
                    SONIC INTELLIGENCE CHECKLIST
                  </h3>
                  ...
                  <div className="space-y-2 text-xs">
                    <div className="flex items-start gap-2 text-gray-400">
                      ...
                      <div className="text-[11px]">
                        <span className="font-semibold text-white">Project Density:</span>{" "}
                        {tracks.length} track channels active. Signal density optimal.
                      </div>
                    </div>
                    <div className="flex items-start gap-2 text-gray-400">
                      <span className="text-[#00FF9C] mt-0.5 font-bold">✔</span>
                      <div className="text-[11px]">
                        <span className="font-semibold text-white">Dynamic range status:</span>{" "}
                        {eqAnalysis.dynamics}
                      </div>
                    </div>
                    <div className="flex items-start gap-2 text-gray-400">
                      ...
                      <div className="text-[11px]">
                        <span className="font-semibold text-white">Harmonic Lock advice:</span>{" "}
                        Scale minors are strictly enforced. Pluck synthesis notes locked to scale.
                      </div>
                    </div>
                  </div>
                </div>
                ...
                const handlePlayPreview = () => {
                  try {
                    ...
                    notesToPlay.forEach((note, index) => {
                      setTimeout(() => {
                        import("../../audio/lowLatencySynth").then(
                          ({
                  ...
                  >
                    <div className="bg-[#121314] rounded-2xl border border-white/5 p-4 relative overflow-hidden shrink-0">
                      ...
                      <p className="text-[11px] text-gray-400 leading-relaxed mb-4">
                        Shape the musical characteristics by blending traits in real-time. Instantly
                        updates timeline notes, drum grooves, and project tempo.
                      </p>
                    ...
                    <div className="bg-[#090A0C] border border-white/5 rounded-2xl p-4 space-y-3.5">
                      <div className="flex items-center justify-between">
                        <span className="text-[10px] font-mono font-bold text-gray-400 uppercase tracking-widest">
                          Active Trait Signatures
                        </span>
                        <span className="text-[9px] font-mono px-2 py-0.5 bg-emerald-500/10 text-[#00FF9C] rounded border border-emerald-500/20 font-bold uppercase">
                          Dynamic
                        </span>
                      </div>
                      ...
                      <div className="space-y-3 font-mono text-xs">
                        ...
                        <div className="space-y-1">
                          <div className="flex justify-between text-[10px]">
                            ...
                            <span className="text-white">
                              {(currentVibe.drumDensity * 100).toFixed(0)}%
                            </span>
                          </div>
                          <div className="h-1 bg-[#1A1C1E] rounded-full overflow-hidden">
                            <div
                              className="h-full bg-teal-400"
                              style={{ width: `${currentVibe.drumDensity * 100}%` }}
                            />
                          </div>
                        </div>
                        ...
                        <div className="space-y-1">
                          <div className="flex justify-between text-[10px]">
                            ...
                            <span className="text-white">
                              {(currentVibe.bassBounce * 100).toFixed(0)}%
                            </span>
                          </div>
                          <div className="h-1 bg-[#1A1C1E] rounded-full overflow-hidden">
                            <div
                              className="h-full bg-pink-500"
                              style={{ width: `${currentVibe.bassBounce * 100}%` }}
                            />
                          </div>
                        </div>
                        ...
                        <div className="space-y-1">
                          <div className="flex justify-between text-[10px]">
                            ...
                            <span className="text-white">
                              {(currentVibe.chordRichness * 100).toFixed(0)}%
                            </span>
                          </div>
                          <div className="h-1 bg-[#1A1C1E] rounded-full overflow-hidden">
                            <div
                              className="h-full bg-amber-400"
                              style={{ width: `${currentVibe.chordRichness * 100}%` }}
                            />
                          </div>
                        </div>
                        ...
                        <div className="space-y-1">
                          <div className="flex justify-between text-[10px]">
                            ...
                            <span className="text-white">
                              {(currentVibe.melodyEnergy * 100).toFixed(0)}%
                            </span>
                          </div>
                          <div className="h-1 bg-[#1A1C1E] rounded-full overflow-hidden">
                            <div
                              className="h-full bg-[#A855F7]"
                              style={{ width: `${currentVibe.melodyEnergy * 100}%` }}
                            />
                          </div>
                        </div>
                        ...
                        <div className="space-y-1">
                          ...
                          <div className="h-1 bg-[#1A1C1E] rounded-full overflow-hidden">
                            <div
                              ...
                              style={{ width: `${currentVibe.fxSpace * 100}%` }}
                            />
                          </div>
                        </div>
                        ...
                        <div className="flex justify-between text-[10px] bg-[#121314] px-2.5 py-2 rounded-lg border border-white/5">
                          ...
                          <span className="text-[#00FF5A] font-extrabold">
                            {currentVibe.tempoBias} BPM
                          </span>
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="bg-[#090A0C] border border-white/5 rounded-2xl p-4 space-y-4">
                      <div className="flex items-center justify-between">
                        <span className="text-[10px] font-mono font-bold text-gray-400 uppercase tracking-widest">
                          Live Studio Console Mixer
                        </span>
                        <span className="text-[9px] font-mono px-2 py-0.5 bg-purple-500/10 text-purple-400 rounded border border-purple-500/20 font-bold uppercase">
                          Smooth Ramped
                        </span>
                      </div>
                      ...
                      <div className="grid grid-cols-2 gap-3">
                        ...
                        <div className="bg-[#121314] rounded-xl p-3 border border-white/5 space-y-2">
                          ...
                          <div className="h-16 flex items-end justify-around gap-2 px-1 relative">
                            ...
                            <div
                              className="w-4 bg-gradient-to-t from-emerald-500 to-teal-400 rounded-t transition-all duration-150 animate-pulse"
                              style={{
                                height: `${Math.max(10, (1 - currentVibe.melodyEnergy) * 100)}%`,
                              }}
                            >
                              <div className="text-[7px] text-zinc-300 font-mono text-center -mt-4">
                                High
                              </div>
                            </div>
                            <div
                              className="w-4 bg-gradient-to-t from-yellow-500 to-amber-400 rounded-t transition-all duration-150 animate-pulse"
                              style={{
                                height: `${Math.max(10, currentVibe.chordRichness * 100)}%`,
                              }}
                            >
                              <div className="text-[7px] text-zinc-300 font-mono text-center -mt-4">
                                Mid
                            ...
                            <div
                              className="w-4 bg-gradient-to-t from-pink-500 to-purple-400 rounded-t transition-all duration-150 animate-pulse"
                              style={{ height: `${Math.max(10, currentVibe.bassBounce * 100)}%` }}
                            >
                              <div className="text-[7px] text-zinc-300 font-mono text-center -mt-4">
                                Bass
                              </div>
                          ...
                          <div className="h-14 flex flex-col justify-around font-mono">
                            ...
                            <div className="flex justify-between text-[8px] text-zinc-500">
                              ...
                              <span className="text-[#A855F7]">
                                Ratio: {((0.5 + currentVibe.drumDensity) * 8).toFixed(1)}:1
                              </span>
                              <span>-24dB</span>
                            </div>
                          </div>
                        ...
                        <div className="bg-[#121314] rounded-xl p-3 border border-white/5 space-y-2 col-span-1">
                          ...
                          <div className="h-14 flex items-center gap-2">
                            <div className="flex-1 flex flex-col gap-1">
                              ...
                              <div className="h-1.5 bg-[#1F2022] rounded-full overflow-hidden">
                                <div
                                  className="h-full bg-red-400/60 transition-all duration-150"
                                  style={{ width: `${currentVibe.drumDensity * 90}%` }}
                                />
                              </div>
                            </div>
                    ...
                    <div className="bg-[#090A0C] border border-white/5 rounded-2xl p-4 space-y-4">
                      <div className="flex items-center justify-between border-b border-white/5 pb-2.5">
                        ...
                        <span className="text-[9px] font-mono px-2 py-0.5 bg-yellow-500/10 text-yellow-400 rounded border border-yellow-500/20 font-bold uppercase">
                          Chorus Impact Engine
                        </span>
                      </div>

                      {/* Goal Description */}
                      <p className="text-[10px] text-zinc-400 leading-relaxed font-mono">
                        Automatically create tension and release before chorus sections to produce
                        professional, high-impact drops.
                      </p>
                      ...
                      <div className="bg-[#121314] rounded-xl p-3 border border-white/5 space-y-1.5">
                        ...
                        <div className="grid grid-cols-5 gap-1.5 pt-1 text-center font-mono">
                          ...
                          <div className="p-1.5 bg-[#161718] rounded border border-white/5">
                            ...
                            <div className="text-[10px] text-teal-400 font-bold font-extrabold">
                              50%
                            </div>
                          </div>
                      ...
                      <div className="flex items-center justify-between bg-[#121314] p-3 rounded-xl border border-white/5">
                        <div className="space-y-0.5">
                          ...
                          <div className="text-[9px] font-mono text-zinc-500">
                            Insert Pre-Chorus Drops automatically
                          </div>
                        </div>
                        <button
                          onClick={() => {
                      ...
                      <div className="bg-[#121314] rounded-xl p-3 border border-white/5 space-y-2.5">
                        <div className="flex justify-between items-center text-[9px] font-mono text-zinc-400 uppercase tracking-wide">
                          ...
                          <span className="text-[#00FF9C]">
                            Playhead: Bar {Math.floor(transportPosition / 16) + 1}
                          </span>
                        </div>
                        ...
                        <div className="space-y-1.5 max-h-48 overflow-y-auto pr-1">
                          {songStructure.sections.map((sec) => {
                            ...
                            const isCurrentlyPlaying =
                              ...
                              currentBar < sec.startBar + sec.lengthBars;
                            ...
                            if (sec.type === "PRE_CHORUS")
                              badgeColor = "text-yellow-400 bg-yellow-500/5 border-yellow-500/10";
                            if (sec.type === "CHORUS")
                              badgeColor = "text-red-400 bg-red-400/5 border-red-500/10";
                            if (sec.type === "OUTRO")
                            ...
                            return (
                              ...
                              >
                                ...
                                <div className="flex items-center gap-2">
                                  <span
                                    className={`text-[8px] font-bold px-1.5 py-0.5 rounded border uppercase ${badgeColor}`}
                                  >
                      ...
                      {(() => {
                        ...
                        const barIdxInSection = activeSection
                          ? currentBar - activeSection.startBar
                          : 0;
                        ...
                        if (activeSection?.type === "PRE_CHORUS") {
                          ...
                          const isSecondHalf =
                            barIdxInSection >= Math.floor(activeSection.lengthBars / 2);
                          if (isSecondHalf) {
                        ...
                        if (activeSection?.type === "PRE_CHORUS") {
                          ...
                          crowdText =
                            barIdxInSection === 0
                              ? "🗣️ COSMIC ROAR DROP HIT OVERFLOW!"
                              : "🕺 Club Shouting & Dancing Active";
                        } else if (activeSection?.type === "INTRO") {
                          ...
                          <div className="space-y-2.5">
                            <div className="bg-[#121314] rounded-xl p-3 border border-white/5 space-y-2">
                              ...
                              <div className="flex items-center justify-between bg-[#161718] p-2.5 rounded border border-white/5">
                                ...
                                <span
                                  className={`text-[10px] font-mono font-extrabold ${mode === "aggressive" ? "text-red-400" : "text-blue-400"}`}
                                >
                                  {mode === "aggressive"
                                    ? "💥 EXPLOSIVE FORCE (Aggressive)"
                                    : "🌊 VELVET SMOOTH (Subtle)"}
                                </span>
                              </div>
                              ...
                              <div className="text-[9px] font-mono text-zinc-500 leading-relaxed bg-[#0c0d0e] p-2 rounded">
                                <span className="text-[#00FF9C] font-bold">DIAGNOSTIC: </span>
                                {message}
                              </div>
                            </div>
                            ...
                            <div className="bg-[#121314] rounded-xl p-3 border border-white/5 space-y-3 font-mono">
                              <div className="text-[9px] text-zinc-400 uppercase tracking-wide w-full flex justify-between">
                                ...
                                <span className="text-yellow-400 animate-pulse">
                                  {activeSection?.type === "PRE_CHORUS" ? "ACTIVE AUTOMATING" : ""}
                                </span>
                              </div>
                                ...
                                <div className="h-1 bg-[#1A1C1E] rounded-full overflow-hidden">
                                  <div
                                    ...
                                    style={{
                                      width:
                                        activeSection?.type === "PRE_CHORUS"
                                          ? `${((barIdxInSection + 1) / activeSection.lengthBars) * 100}%`
                                          : "5%",
                                    }}
                                  />
                                </div>
                              ...
                              <div className="flex items-center justify-between text-[9px] bg-[#161718] p-1.5 rounded border border-white/5">
                                ...
                                <span
                                  className={`font-bold px-1 py-0.5 rounded text-[8px] uppercase ${
                                    activeSection?.type === "PRE_CHORUS"
                              ...
                              <div className="flex items-center justify-between text-[9px] bg-[#161718] p-1.5 rounded border border-white/5">
                                ...
                                <span
                                  className={`font-bold text-[9px] ${activeSection?.type === "PRE_CHORUS" ? "text-yellow-400" : "text-zinc-500"}`}
                                >
                                  {activeSection?.type === "PRE_CHORUS"
                              ...
                              <div className="flex items-center justify-between text-[9px] bg-[#161718] p-1.5 rounded border border-white/5">
                                ...
                                <span
                                  className={`font-bold px-1.5 py-0.5 rounded text-[8px] uppercase ${
                                    activeSection?.type === "PRE_CHORUS" &&
                                    barIdxInSection === activeSection.lengthBars - 1
      ...
      <div
        className={`p-4 z-10 pointer-events-auto relative transition-all duration-300 ${pipelineActive ? "border-t-0 bg-transparent" : "border-t border-[#1A1A1A] bg-[#0E0F10]"}`}
      >
          ...
          {pipelineActive ? (
            <div className="w-full flex items-center justify-end">
              <button
                onClick={() => {
                  ...
                  addChatMessage({
                    role: "assistant",
                    content: "🚫 Pipeline aborted by User override.",
                  });
                }}
            ...
            <div className="w-full relative">
              <input
                ...
                placeholder={
                  isLoading
                    ? "AI is working, calculated credit being spent..."
                    : adRequiredPrompt
                      ? "Ad watching verification required to unlock..."
                      : "Ask AI or command physical operations (e.g. 'produce full Afrobeat song' or 'mix down tracks')"
                }
              ...
              <button
                ...
                className={`absolute right-2 top-2 bottom-2 w-10 h-10 rounded-xl flex items-center justify-center transition-all ${
                  input.trim() && !isLoading && !adRequiredPrompt
                    ? "bg-[#00FF9C] text-black hover:scale-105 active:scale-95"
        ...
        <div className="mt-3 flex flex-wrap gap-1.5 justify-center">
          {[
            {
              icon: Sparkle,
              text: "Produce Full Song",
              action: "produce full Afrobeat song automatically",
            },
            {
              icon: SlidersHorizontal,
              text: "Auto-Mix Channels",
              action: "mix down active tracks",
            },
            {
              icon: Sparkles,
              text: "Process Vocal EQ",
              action: "apply warm DSP tuning & autotune on vocals",
            },
            {
              icon: Download,
              text: "Export HighFi Masters",
              action: "render and export WAV MP3 files",
            },
          ].map((s, idx) => (
            <button
              key={idx}
